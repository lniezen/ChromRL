...
class Separation:
    ...
    def __init__(
        ...
        eluted = np.where(self.loc <= 20.0)[0]

        self._eluted = len(eluted)
        self._uneluted = len(self.loc) - len(eluted)

        self.loc_all = np.nan_to_num(self.loc, nan=0.0)
        self.loc_all = np.where(self.loc_all > 20.0, 0.0, self.loc_all)
        self.scale_all = self.scale
...
class ChromatographicResponseFunction:
    ...
    def _scale_resolution(self, resolution):
        return resolution / self.resolution_target
