...
class Chromatography(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Dict(
            {
                'elution_time': gym.spaces.Box(
                    low=-0.0,
                    high=1.0,
                    shape=(np.max(num_compounds), ),
                    dtype=self._dtype),
                'elution_width': gym.spaces.Box(
                    low=0.0,
                    high=1.0,
                    shape=(np.max(num_compounds),),
                    dtype=self._dtype),
    ...
    def reset(self, **kwargs) -> tuple:
        ...
        elution_time = separation.loc_all
        elution_width = separation.scale_all
        observation = {
            'elution_time': elution_time / self.max_exp_time,
            'elution_width': elution_width,
